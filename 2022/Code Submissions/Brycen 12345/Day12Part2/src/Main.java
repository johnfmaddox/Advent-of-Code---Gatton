...
public class Main {
    public static void main(String[] args) {
        final String fileName = "Day12Part2/Data.txt";
        ...
        try{
            ...
            List<QueueNode> starts = new ArrayList<>();
            for(int row = 0; row < mountains.size(); row++) {
                for(int column = 0; column < mountains.get(row).size(); column++) {
                    if(mountains.get(row).get(column) == 83 || mountains.get(row).get(column) == 97) {
            ...
            for(QueueNode s : starts) {
                int ans = 0;
                ans = shortest(mountains, s);
                if(ans > 0) {
                    paths.add(ans);
                }
            }
            ...
            //System.out.println(mountains.get(20).get(0));
            /*
    ...
    public static int shortest(List<List<Integer>> grid, QueueNode start) {
